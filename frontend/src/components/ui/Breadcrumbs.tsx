  ...
  return (
    ...
    >
        ...
        <div key={index} className="flex items-center">
          ...
          {!item.href || index === items.length - 1 ? (
            ...
            >
              ...
              <span className="truncate max-w-[200px] group-hover:translate-x-0.5 transition-transform duration-300">
                {item.label}
              </span>
            </Link>
          )}
      ...
      {current && current !== items[items.length - 1]?.label && (
        <>
          <ChevronRight className="w-4 h-4 text-gray-500 mx-2 flex-shrink-0" />
          <span className="glass-button-small px-3 py-1.5 text-white font-semibold rounded-xl truncate max-w-[200px]">
            {current}
          </span>
        </>
      )}
